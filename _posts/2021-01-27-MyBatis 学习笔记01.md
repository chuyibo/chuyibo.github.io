...
创建持久层接口，持久层接口的名称必须与持久层映射文件名称保持一致，且持久层接口与持久层映射必须在相同的包路径下。
...
编写测试代码，根据全局配置文件利用 SqlSessionFactoryBuilder 创建 SqlSessionFactory，使用 SqlSessionFactory 获取 SqlSession，一个 SqlSession 代表和数据库的一次会话。使用 SqlSession 生产持久层接口的代理对象对数据库进行操作。SqlSession 不是线程安全的，每次使用完成后都需要关闭。
