---
title: MyBatis 增、删、改、查操作
indextitle: MyBatis 学习笔记(三)
date: 2021-01-27 23:30:09
description: MyBatis 增、删、改、查、模糊查询示例。查询数据时映射文件中占位符的使用说明，在模糊查询中拼接符的使用说明。
tags:
- MyBatis
---

<p align="center">查询操作</p>

---

1.持久层的接口

```
public interface IUserDao {

    User selectByUserId(int userId);
}
```

2.持久层的映射

```
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <select id="selectByUserId" resultType="chu.yi.bo.domain.User" parameterType="int">
		SELECT * FROM user WHERE id = #{uid};
	</select>
</mapper>
```

3.查询数据示意

```
User user = userDao.selectByUserId(userId);
```

Select 标签的 Id 属性值必须与持久层接口的方法名一致。ResultType 属性用于指定 SQL 执行的结果集类型，这里使用的是全限定类名。ParameterType 属性用于指定传入参数的类型。占位符（#{} 符号）在执行 SQL 时会被实际数据替换，具体的数据由占位符里面的内容决定。

<p align="center">保存操作</p>

----

1.持久层的接口

```
public interface IUserDao {

    int saveUser(User user);
}
```

2.持久层的映射

```
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <insert id="saveUser" parameterType="chu.yi.bo.domain.User">
		INSERT INTO user (username, age, sex) VALUES (#{username},#{age},#{sex});
	</insert>
</mapper>
```

3.保存数据示意

```
User user = new User();

// 填充数据
user.set...();
userDao.saveUser(user);
...
// 保存数据一定要提交事务
session.commit();
```

ParameterType 属性指定了参数类型，这里使用全限定类名，因此参数是一个 User 对象。占位符（语法：#{}）中可以使用 OGNL 表达式（语法：{对象.对象}），来获取对象的属性值。因为已经指定了参数类型，在占位符中直接使用属性名，SQL 执行时就会去 User 对象中获取对应的属性值。

<p align="center">更新操作</p>

----

1.持久层的接口

```
public interface IUserDao {

    User selectByUserId(int userId);
	int updateUser(User user);
}
```

2.持久层的映射

```
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <select id="selectByUserId" resultType="chu.yi.bo.domain.User" parameterType="int">
    	SELECT * FROM user WHERE id = #{uid};
	</select>

	<update id="updateUser" parameterType="chu.yi.bo.domain.User">
    	UPDATE user SET username=#{username}, age=#{age}, sex=#{sex} WHERE id=#{uid};
	</update>
</mapper>
```

3.更新数据示意

```
// 先查询再更新
User user = userDao.findById(userId);

// 重置数据
user.set...();
userDao.updateUser(user);
...
// 更新数据一定要提交事务
session.commit();
```

<p align="center">删除操作</p>

----

1.持久层的接口

```
public interface IUserDao {

    int deleteByUserId(Integer userId);
}
```

2.持久层的映射

```
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <delete id="deleteByUserId" parameterType="int">
    	DELETE FROM user WHERE id = #{uid};
	</delete>
</mapper>
```

3.数据删除示意

```
userDao.deleteByUserId(userId);
```

<p align="center">模糊查询</p>

----

1.持久层的接口

```
public interface IUserDao {

    List<User> selectByUsername(String username);
}
```

2.持久层的映射

```
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <select id="selectByUsername" resultType="chu.yi.bo.domain.User" parameterType="string">
    	select * from user where username like '%${value}%';
	</select>
</mapper>
```

3.模糊查询示意

```
List<User> users = userDao.findByName(userName);
```

拼接符（${} 符号）在字符串中使用，在 SQL 执行时，拼接符在字符串中的位置会被参数替代，拼接符中的变量名称只能使用 value。