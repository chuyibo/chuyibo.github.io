---
title: MyBatis 动态 SQL
indextitle: MyBatis 学习笔记(八)
date: 2021-01-27 23:30:09
description: 
tags:
- MyBatis
---

<p align="center">if 标签使用</p>

---

```
持久层接口
public interface IUserDao {

    查询用户信息
    public List<User> queryUserList(User user);
}
```

```
<!-- 持久层接口 -->
<mapper namespace="chu.yi.bo.dao.IUserDao">
    <select id="queryUserList" resultType="chu.yi.bo.domain.User" parameterType="chu.yi.bo.domain.User">
        SELECT * FROM user WHERE 1=1
        <if test="username != null and username != ''">
            AND username LIKE '%${username}%'
        </if>
        <if test="sex != null and sex != ''">
            AND sex = #{sex}
        </if>
    </select>
</mapper>
```

查询用户列表，如果 username 不为空根据 username 模糊查询，为空则根据 sex 性别查询。where 1=1 是为了防止 username 和 sex 都为空。if 标签 test 属性值可以使用OGNL 表达式(语法：#{对象.对象})，来获取对象的属性值。因为已经指定了参数类型，示例中参数是一个 User 对象，可以直接使用属性名，SQL 执行时就会去 User 对象中获取对应的属性值。